    ...
    ***************************************************************************** */
    ...
    function normalizeDateValue(dateValue, _a) {
        ...
        return value && value.toString() === dateValue
            ? dateValue
            : cdk.TuiDay.normalizeParse(dateValue).dayLimit(min, max).toString();
    }
    ...
    function parseWithLimit(value, config) {
        return cdk.TuiDay.normalizeParse(value.slice(0, config.filler.length)).dayLimit(config.min, config.max);
    }
    function processRawValue(value, config) {
        switch (value.length) {
            case config.filler.length:
                return parseWithLimit(value, config).toString();
            case config.filler.length + cdk.RANGE_SEPARATOR_CHAR.length:
                return parseWithLimit(value, config).toString() + cdk.RANGE_SEPARATOR_CHAR;
            case config.rangeFiller && config.rangeFiller.length:
                return config.value && config.value.toString() === value
                    ? value
                    : cdk.TuiDayRange.sort(parseWithLimit(value.slice(0, config.filler.length), config), parseWithLimit(value.slice(config.filler.length + cdk.RANGE_SEPARATOR_CHAR.length), config)).toString();
            default:
                return value;
        }
    }
    ...
    function tuiCreateTimeMask(mode) {
        return __spread(constants.TUI_TIME_MASK, (mode.includes('HH:MM:SS') ? [':', core.TUI_DIGIT_REGEXP, core.TUI_DIGIT_REGEXP] : []), (mode === 'HH:MM:SS.MSS'
            ? ['.', core.TUI_DIGIT_REGEXP, core.TUI_DIGIT_REGEXP, core.TUI_DIGIT_REGEXP]
            : []));
    }
